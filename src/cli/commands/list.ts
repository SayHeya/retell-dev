...
/**
 * List agents from a Retell workspace
 */
async function listWorkspaceAgents(workspace: WorkspaceType): Promise<void> {
  ...
  const agents = listResult.value as Array<{
    agent_id: string;
    agent_name?: string;
    voice_id: string;
    language?: string;
  ...
  // Display table
  const table = new Table({
    head: ['Agent ID', 'Name', 'Voice', 'Language', 'LLM ID'],
    colWidths: [35, 30, 20, 12, 35],
  });
  ...
  for (const agent of agents) {
    table.push([
      ...
      agent.voice_id,
      agent.language || '(default)',
      agent.response_engine?.llm_id || '(none)',
    ]);
  }
